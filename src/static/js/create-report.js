...
const element = elById("toggle-address");
element.addEventListener("change", function(event) {
	if (element.checked) {
		let addressRaw = element.nextSibling.textContent;
		let splitted = addressRaw.split(": ");
		let address = splitted[1].trim().replace(/\n/g, "").replace(/\t/g, "");
		elByName("address").value = address;
	} else {
		elByName("address").value = '';
	};
});
...
const category = elByName("category");
